...
test("config: features.combos=false → combos fetcher never called", async () => {
  ...
  const provider = (input as { provider: Record<string, OmniRouteStaticProviderEntry> }).provider;
  assert.ok(provider["opencode-omniroute"].models["claude-sonnet-4-6"], "models block intact");
  assert.equal("claude-tier" in provider["opencode-omniroute"].models, false, "no combo rows");
});
